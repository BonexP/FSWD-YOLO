...
### ✅ 5. 集成测试
- **文件**: `test_shapeiou_integration.py`
- **测试内容**:
  - ShapeIoU 函数基本功能
  - 不同 scale 参数的影响
  - xywh/xyxy 格式支持
...
### 基本训练（使用默认 CIoU）
```bash
python train.py --cfg data.yaml --epochs 100 --batch 16
```

### 使用 ShapeIoU
```bash
python train.py --cfg data.yaml --epochs 100 --batch 16 --iou-type ShapeIoU
```
...
python train.py --epochs 20 --augment --name test_shapeiou --weighted-dataloader
```
