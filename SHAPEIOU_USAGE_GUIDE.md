...
#### 2.1 使用默认的 CIoU（无需指定）
```bash
python train.py --cfg data.yaml --epochs 100
```

#### 2.2 使用 ShapeIoU
```bash
python train.py --cfg data.yaml --epochs 100 --iou-type ShapeIoU
```

#### 2.3 使用其他 IoU 类型
```bash
# GIoU
python train.py --cfg data.yaml --epochs 100 --iou-type GIoU

# DIoU
python train.py --cfg data.yaml --epochs 100 --iou-type DIoU

# 标准 IoU
python train.py --cfg data.yaml --epochs 100 --iou-type IoU
```
...
```bash
# 只指定部分参数，其他使用默认值
python train.py --epochs 20 --name test_run

# 使用增强和加权数据加载器
python train.py --epochs 20 --augment --weighted-dataloader --name test_shapeiou

# 自定义 IoU 类型
python train.py --epochs 50 --iou-type ShapeIoU --name shapeiou_test
```
...
```bash
# 运行对比实验
python train.py --cfg data.yaml --epochs 100 --iou-type CIoU --name exp_ciou
python train.py --cfg data.yaml --epochs 100 --iou-type ShapeIoU --name exp_shapeiou
...
### 运行完整集成测试
```bash
python test_shapeiou_integration.py
```
...
- [ ] 运行 `python check_shapeiou_status.py` 显示 100% 通过
- [ ] 运行 `python test_shapeiou_integration.py` 全部测试通过
- [ ] 训练时能看到 IoU 类型的配置信息
...
**开始训练**:
```bash
python train.py --cfg data.yaml --epochs 100 --iou-type ShapeIoU --name my_experiment
```
