...
> 💡 **提示**：完整的实现代码在 `scripts/custom_loss_example.py` 文件中
...
完整的实现示例已保存在：
- **`scripts/custom_loss_example.py`** - 包含所有自定义 IoU 函数和使用示例

运行示例：
```bash
python scripts/custom_loss_example.py
```
...
✅ **文件参考**：
- 查看 `scripts/custom_loss_example.py` 了解完整实现
