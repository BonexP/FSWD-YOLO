...
```bash
python scripts/train.py --epochs 20 --augment --name test_shapeiou --weighted-dataloader --iou-type ShapeIoU
```
...
- ✅ `scripts/train.py` - 命令行参数 `--iou-type`
...
```bash
# 使用默认 CIoU
python scripts/train.py --epochs 100 --cfg data.yaml

# 使用 ShapeIoU
python scripts/train.py --epochs 100 --cfg data.yaml --iou-type ShapeIoU

# 使用 GIoU
python scripts/train.py --epochs 100 --cfg data.yaml --iou-type GIoU
```
...
3. ✅ 在 `scripts/train.py` 中添加命令行参数（可选）
4. ✅ 更新文档说明参数用途

### 为什么之前没发现这个问题？

- 运行 `tests/test_shapeiou_integration.py` 时：直接调用函数，不经过配置验证
