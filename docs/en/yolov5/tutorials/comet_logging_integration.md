...
python scripts/train.py --img 640 --batch 16 --epochs 5 --data coco128.yaml --weights yolov5s.pt
```
...
```bash
env COMET_MAX_IMAGE_UPLOADS=200 python scripts/train.py \
  --img 640 \
  --batch 16 \
  --epochs 5 \
  --data coco128.yaml \
...
```bash
env COMET_LOG_PER_CLASS_METRICS=true python scripts/train.py \
  --img 640 \
  --batch 16 \
  --epochs 5 \
  --data coco128.yaml \
...
The `hpo.py` script accepts the same arguments as `scripts/train.py`. If you wish to pass additional arguments to your sweep simply add them after the script:
